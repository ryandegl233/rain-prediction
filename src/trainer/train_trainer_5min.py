...
class RainTSNextFrameTrainer:
    ...
    def _next_prediction_loss(
        ...
        def build_rain_regression_loss_map(
            ...
            # 1. 🛡️ 核心防御：白名单扩展，允许 'mae' 顺利通过校验
            if mode not in ["mse", "mae", "enhanced"]:
                raise ValueError(f"train.loss.mode must be 'mse', 'mae' or 'enhanced', got {mode}")

            # 2. 🌟 针对 MAE (L1) 模式的特判分支：直接计算并返回绝对误差
            if mode == "mae":
                abs_error = F.l1_loss(pred_rain, target_rain, reduction="none")
                return abs_error, False
        ...
        def build_rain_event_loss_map(
            ...
            loss_cfg = self.train_cfg.get("loss", {})  # 这里的 self 会自动从外层函数捕获，无需在参数列表中声明
            mode = str(loss_cfg.get("mode", "mse")).lower()
        ...
        if not sequence_loss_enabled:
            ...
            rain_loss_map, rain_weighted_enabled = build_rain_regression_loss_map(pred["rain"], target_gt["rain"])
            l_rain = rain_loss_map.mean()
            event_map, event_weight = build_rain_event_loss_map(pred["rain"], target_gt["rain"])
        ...
        rain_map, rain_weighted_enabled = build_rain_regression_loss_map(pred["rain"], target_gt["rain"])
        rain_event_map, event_weight = build_rain_event_loss_map(pred["rain"], target_gt["rain"])
        ...
        loss = lw_radar * l_radar + lw_satellite * l_satellite + lw_rain * l_rain + rain_event
    ...
    def train_step(self, batch: dict[str, torch.Tensor]) -> tuple[dict[str, torch.Tensor], bool]:
        ...
        # 1. 🌟 记录最原始真实的未来帧数，用于最后的损失计算和裁剪
        raw_target_frames = int(aux["target_frames"])
        ...
        # 2. 🛡️ 核心整除约束防御 1：确保 context 的时间轴 T 能够被 2 整除
        if context.shape[2] % 2 != 0:
            context = F.pad(context, (0, 0, 0, 0, 0, 1), mode='replicate')
            if torch.is_tensor(context_time):
                context_time = F.pad(context_time, (0, 1), mode='replicate')

        # 3. 🛡️ 核心整除约束防御 2：如果切片得出的未来真值为奇数帧（如1），全方位深度对齐成偶数帧（如2）
        if raw_target_frames % 2 != 0:
            target_frames = raw_target_frames + 1
            aux["target_frames"] = target_frames  # 重写字典数值，绕过模型内部校验

            # 【深度校准】如果 target_seed_time 长度小于新帧数，同步在末尾复制填充
            if torch.is_tensor(target_seed_time) and target_seed_time.shape[1] < target_frames:
                target_seed_time = F.pad(target_seed_time, (0, target_frames - target_seed_time.shape[1]),
                                         mode='replicate')

            # 【核心补充】部分模型会强校验 target_seed 在 T 轴的帧数与目标帧数关系，这里在 T 轴填充至相同大小
            if torch.is_tensor(target_seed) and target_seed.shape[2] < target_frames:
                padding_len = target_frames - target_seed.shape[2]
                target_seed = F.pad(target_seed, (0, 0, 0, 0, 0, padding_len), mode='replicate')
        else:
            target_frames = raw_target_frames
        ...
        with self.accelerator.accumulate(self.model):
            with self.accelerator.autocast():
                ...
                # 模型自回归前向传播预测（此时所有字典、时间、输入 Tensor 全方位对齐为 2 的倍数）
                pred = self.model.forward_ar(**forward_kwargs)

                # 4. 🌟 结果裁剪：无论为了迎合模型多预测了多少帧，都裁剪回最原始的真实帧数用于计算 Loss
                pred_clipped = {}
                for k, v in pred.items():
                    if isinstance(v, torch.Tensor):
                        pred_clipped[k] = v[:, :, :raw_target_frames]
                    else:
                        pred_clipped[k] = v

                # 5. 还原 aux 字典中的数值，确保下一步的 loss 函数和 logger 能获得原始标签
                aux["target_frames"] = raw_target_frames

                # 1. 基础教师逼近重建损失 (像素级回归损失，此时已从 YAML 同步为 MAE 损失)
                rec_loss, logs = self._next_prediction_loss(pred=pred_clipped, target_gt=target_gt, aux=aux)

                # =========================================================================
                # 🔥 新增注入：SSIM 结构相似性损失 (强迫模型关注雨团结构形态与融合趋势)
                # =========================================================================
                try:
                    from pytorch_msssim import ssim
                    ssim_loss = 0.0
                    for modality in ["radar", "satellite", "rain"]:
                        p_tensor = pred_clipped[modality]  # [B, C, T, H, W]
                        g_tensor = target_gt[modality]  # [B, C, T, H, W]

                        # SSIM 期望输入为 4D 空间 [N, C, H, W]，此处将 Batch 与 Time 维度合并处理
                        B, C, T_dim, H, W = p_tensor.shape
                        p_4d = p_tensor.transpose(1, 2).reshape(B * T_dim, C, H, W)
                        g_4d = g_tensor.transpose(1, 2).reshape(B * T_dim, C, H, W)

                        # 约束数值范围在 1.0 以内，计算结构失真度 (1.0 - ssim)
                        modality_ssim = ssim(p_4d, g_4d, data_range=1.0, size_average=True)
                        ssim_loss += (1.0 - modality_ssim)

                    # 设定 SSIM 损失约束权重
                    lambda_ssim = 0.5
                    rec_loss = rec_loss + lambda_ssim * ssim_loss
                    logs["loss/ssim"] = ssim_loss.detach()
                except ImportError:
                    # 容错处理：若环境中缺少 pytorch_msssim 库则仅跳过，不阻断正常运行
                    pass
                # =========================================================================

                # =========================================================================
                # 🔥 核心防御升级：运动趋势与防复制差分损失 (Anti-Collapse Temporal Loss)
                # =========================================================================
                loss_cfg = self.train_cfg.get("loss", {})
                td_cfg = loss_cfg.get("temporal_diff", {})

                if bool(td_cfg.get("enabled", False)):
                    diff_loss = 0.0
                    for modality in ["radar", "satellite", "rain"]:
                        p_tensor = pred_clipped[modality]  # [B, C, T, H, W]
                        g_tensor = target_gt[modality]  # [B, C, T, H, W]

                        if p_tensor.shape[2] > 1:
                            # 一阶运动差分（原本的逻辑：促使预测和真值的变化趋势一致）
                            pred_diff1 = p_tensor[:, :, 1:] - p_tensor[:, :, :-1]
                            gt_diff1 = g_tensor[:, :, 1:] - g_tensor[:, :, :-1]
                            diff_loss += F.l1_loss(pred_diff1, gt_diff1)

                            # 🛡️ 核心防复制拦截：计算二阶运动差分（惩罚绝对静止复制）
                            if p_tensor.shape[2] > 2:
                                pred_diff2 = pred_diff1[:, :, 1:] - pred_diff1[:, :, :-1]
                                gt_diff2 = gt_diff1[:, :, 1:] - gt_diff1[:, :, :-1]
                                diff_loss += 0.5 * F.l1_loss(pred_diff2, gt_diff2)
                        else:
                            # 只有 1 帧长时的时序差分兜底机制
                            anchor_tensor = aux["temporal_diff_anchor"][modality]
                            pred_diff = p_tensor - anchor_tensor
                            gt_diff = g_tensor - anchor_tensor
                            diff_loss += F.l1_loss(pred_diff, gt_diff)

                    lambda_diff = float(td_cfg.get("lambda_diff", 2.0))
                    rec_loss = rec_loss + lambda_diff * diff_loss
                    logs["loss/temporal_difference"] = diff_loss.detach()
                # =========================================================================

                # 2. 闭环外推分支损失（内部已加上针对训练集 batch is None 的防御逻辑）
            ...
            if self.use_gan:
                ...
                target_tensor = self._merge_modalities(target_gt["radar"], target_gt["satellite"], target_gt["rain"])
                pred_tensor = self._merge_modalities(pred_clipped["radar"], pred_clipped["satellite"],
                                                     pred_clipped["rain"])
        ...
        target_mode_str = str(aux.get("target_mode", "next_frame"))
        target_mode_map = {"next_frame": 0.0, "block": 1.0, "sequence": 2.0}
        logs["meta/target_mode"] = torch.tensor(
            target_mode_map.get(target_mode_str, -1.0), device=self.device
        )

        logs["meta/target_frames"] = torch.tensor(float(raw_target_frames), device=self.device)
    ...
    def val_step(self, batch: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        # 1. 🛡️ 确保历史上下文 context 的时间轴 T 能够被 2 整除
        if context.shape[2] % 2 != 0:
            context = F.pad(context, (0, 0, 0, 0, 0, 1), mode='replicate')
            if torch.is_tensor(context_time):
                context_time = F.pad(context_time, (0, 1), mode='replicate')

        # 2. 🛡️ 核心修复：如果未来帧数是奇数，全方位升级并对齐为偶数帧，防止模型内部断言冲突
        if raw_target_frames % 2 != 0:
            target_frames = raw_target_frames + 1
            aux["target_frames"] = target_frames  # 临时改写字典状态，通过模型内部对齐校验

            # 【校准时间戳】将未来时间序列长度同步补齐到对齐后的偶数帧数
            if torch.is_tensor(target_seed_time) and target_seed_time.shape[1] < target_frames:
                target_seed_time = F.pad(target_seed_time, (0, target_frames - target_seed_time.shape[1]),
                                         mode='replicate')

            # 【校准引导种子】核心补丁！将引导预测的种子序列在 T 轴上也用边界复制同步补齐到偶数帧数
            if torch.is_tensor(target_seed) and target_seed.shape[2] < target_frames:
                padding_len = target_frames - target_seed.shape[2]
                target_seed = F.pad(target_seed, (0, 0, 0, 0, 0, padding_len), mode='replicate')
        else:
        ...
        with self.accelerator.autocast():
            forward_kwargs = {
                "context_x": context,
                "target_x": target_seed,
                "predict_frames": target_frames,  # 此时传入的帧数、时间序列长度、引导种子长度完美等长（皆为 2）
                "strict_target_isolation": bool(self.train_cfg.strict_target_isolation),
                "return_modality_dict": True,
            }
            if context_modality_available is not None:
                forward_kwargs["context_modality_available"] = context_modality_available
            if torch.is_tensor(context_time):
                forward_kwargs["context_time"] = context_time
            if torch.is_tensor(target_seed_time):
                forward_kwargs["target_time"] = target_seed_time

            # 此时前向传播输入完全自洽，不会再触发 time length mismatch 报错
            pred = self.model.forward_ar(**forward_kwargs)

            # 3. 🌟 结果无感裁剪：无论为了网络结构多预测了多少帧，都在送入损失函数前裁剪回最真实的奇数帧
            pred_clipped = {}
            for k, v in pred.items():
                if isinstance(v, torch.Tensor):
                    pred_clipped[k] = v[:, :, :raw_target_frames]
            ...
            # 4. 彻底还原 aux 字典中的 target_frames 真实值，确保下游计算基于真实的 GT 进行
            aux["target_frames"] = raw_target_frames

            # 使用裁剪对齐后的纯正 1 帧预测计算验证集的 Reconstruction Loss
            _, logs = self._next_prediction_loss(pred=pred_clipped, target_gt=target_gt, aux=aux)
    ...
    def _prepare_val_inference_batch(
        ...
        if "time_past" in batch and "time_future" in batch:
            ...
            if int(context_time.shape[0]) != int(context.shape[0]) or int(target_time.shape[0]) != int(
                    context.shape[0]):
                raise ValueError(
            ...
            if int(context_time.shape[1]) != int(context.shape[2]) or int(target_time.shape[1]) != int(
                    target["rain"].shape[2]):
                raise ValueError(
    ...
    def _prepare_val_inference_batch(self, batch: dict[str, torch.Tensor]):
        # 1. 提取用于初始外推的 Past Context
        radar_past = self._ensure_bcthw(batch["radar_past"].to(self.device), self.radar_c, "radar_past")
        satellite_past = self._ensure_bcthw(batch["satellite_past"].to(self.device), self.satellite_c, "satellite_past")
        rain_past = self._ensure_bcthw(batch["rain_past"].to(self.device), self.rain_c, "rain_past")

        # =========================================================================
        # 🟢 帧数对齐修补：解决 context_frames 无法被 frame_patch_size 整除的问题
        # =========================================================================
        # 尝试动态获取模型的 patch 帧数大小，获取不到则默认为 2
        frame_patch_size = getattr(self, "_resolve_frame_patch_size", lambda: 2)()

        # 检查是否需要对 Past 帧进行 Padding (对齐时间轴的第 2 维：B, C, T, H, W)
        current_frames = radar_past.shape[2]
        if current_frames % frame_patch_size != 0:
            remainder = current_frames % frame_patch_size
            pad_len = frame_patch_size - remainder

            # 使用 replicate 模式复制最后一帧进行填充 (分别对应 W, H, T 维度的左右 padding)
            radar_past = F.pad(radar_past, (0, 0, 0, 0, 0, pad_len), mode='replicate')
            satellite_past = F.pad(satellite_past, (0, 0, 0, 0, 0, pad_len), mode='replicate')
            rain_past = F.pad(rain_past, (0, 0, 0, 0, 0, pad_len), mode='replicate')
        ...
        if "rain_future" in batch:
            target = {
                "radar": self._ensure_bcthw(batch["radar_future"].to(self.device), self.radar_c, "radar_future"),
                "satellite": self._ensure_bcthw(batch["satellite_future"].to(self.device), self.satellite_c,
                                                "satellite_future"),
                "rain": self._ensure_bcthw(batch["rain_future"].to(self.device), self.rain_c, "rain_future"),
            }
        else:
            b, c, _, h, w = context.shape
            target = {
                "radar": torch.zeros(b, self.radar_c, 1, h, w, device=self.device),
                "satellite": torch.zeros(b, self.satellite_c, 1, h, w, device=self.device),
                "rain": torch.zeros(b, self.rain_c, 1, h, w, device=self.device),
            }

        # 3. 提取并对齐时间步信息
        context_time = batch["context_time"].to(self.device) if "context_time" in batch else None
        target_time = batch["target_time"].to(self.device) if "target_time" in batch else None

        # 如果时间戳存在，也必须同步做复制 Padding 保证时序长度对齐
        if context_time is not None and context_time.shape[1] < context.shape[2]:
            t_pad = context.shape[2] - context_time.shape[1]
            context_time = F.pad(context_time, (0, t_pad), mode='replicate')
    ...
    def _val_inference_step(
            self,
            batch: dict[str, torch.Tensor]
    ) -> tuple[dict[str, torch.Tensor], dict[str, torch.Tensor] | None, torch.Tensor | None, dict[str, torch.Tensor]]:
        ...
        # =========================================================================
        # 🛡️ 终极近身防御：在读取 shape[2] 的前一刻死守拦截，彻底斩断 NoneType 报错
        # =========================================================================
        if (target is None
                or not isinstance(target, dict)
                or "rain" not in target
                or target["rain"] is None):

            try:
                target_frames = int(aux.get("target_frames", 1))
            except:
                target_frames = 1

            # 💡 修复：使用各自模态正确的通道数 (self.radar_c 等) 制造安全的沙盒字典，防止后续指标计算维度不匹配
            b = context.shape[0] if context is not None else 1
            h, w = (context.shape[3], context.shape[4]) if context is not None else (256, 256)

            target = {
                "radar": torch.zeros(b, getattr(self, "radar_c", 1), target_frames, h, w, device=self.device),
                "satellite": torch.zeros(b, getattr(self, "satellite_c", 1), target_frames, h, w, device=self.device),
                "rain": torch.zeros(b, getattr(self, "rain_c", 1), target_frames, h, w, device=self.device)
            }
            has_valid_gt = False
        else:
            has_valid_gt = True

        # 此时 target["rain"] 必定存在，100% 安全
        ...
        # 3. 🛡️ 奇数帧维度整除保护（避免 Causal Patch Transformer 内部断言失败）
        raw_target_frames = target_frames
        if context is not None and context.shape[2] % 2 != 0:
            context = F.pad(context, (0, 0, 0, 0, 0, 1), mode='replicate')
            if torch.is_tensor(context_time):
                context_time = F.pad(context_time, (0, 1), mode='replicate')
        ...
        with self.accelerator.autocast():
            ...
            for k, v in pred.items():
                if isinstance(v, torch.Tensor):
                    pred_target[k] = v[:, :, :raw_target_frames]
                else:
                    pred_target[k] = v
        ...
        if has_valid_gt:
            ...
            lw_radar = float(getattr(lw, "radar", 1.0))
            lw_satellite = float(getattr(lw, "satellite", 1.0))
            lw_rain = float(getattr(lw, "rain", 1.0))

            # 🔴 核心修改：将硬编码的 F.mse_loss 彻底替换为 F.l1_loss (MAE)，严打验证阶段的过度虚报
            import torch.nn.functional as F
            infer_loss = (
                    lw_radar * F.l1_loss(pred_target["radar"], target["radar"])
                    + lw_satellite * F.l1_loss(pred_target["satellite"], target["satellite"])
                    + lw_rain * F.l1_loss(pred_target["rain"], target["rain"])
            )
            extra_logs["loss/val_inference"] = infer_loss.detach()
        else:
            infer_loss = torch.tensor(0.0, device=self.device)
            extra_logs["loss/val_inference"] = infer_loss.detach()

        # 💡 关键修复：必须明确 return 这四个变量，维持下游解包稳定性
    ...
    def _train_rollout_branch_loss(
        ...
        # =========================================================================
        # 🛡️ 核心修复：如果 target 为 None，从原始 batch 中手动提取未来真值并规范化维度
        # =========================================================================
        if target is None:
            target = {
                "radar": batch["radar_future"].to(self.device, dtype=torch.float32),
                "satellite": batch["satellite_future"].to(self.device, dtype=torch.float32),
                "rain": batch["rain_future"].to(self.device, dtype=torch.float32),
            }
            # 确保转换为标准的 (B, C, T, H, W) 5D Tensor 形状
            target["radar"] = self._ensure_bcthw(target["radar"], self.radar_c, "radar_future")
            target["satellite"] = self._ensure_bcthw(target["satellite"], self.satellite_c, "satellite_future")
            target["rain"] = self._ensure_bcthw(target["rain"], self.rain_c, "rain_future")

            # 如果 target_time 也是 None，尝试从 batch 中恢复
            if target_time is None and "target_time" in batch:
                target_time = batch["target_time"].to(self.device)
        # =========================================================================
    ...
    def _predict_next_after_roll_block(
        ...
        lw = self.train_cfg.loss_weights
        loss_next = (
            lw_radar * F.mse_loss(pred_next["radar"], target_next["radar"])
            + lw_satellite * F.mse_loss(pred_next["satellite"], target_next["satellite"])
            + lw_rain * F.mse_loss(pred_next["rain"], target_next["rain"])
        )
    ...
    def _val_inference_step(
        ...
        lw = self.train_cfg.loss_weights
        lw_radar = getattr(lw, "radar", 1.0)
        lw_satellite = getattr(lw, "satellite", 1.0)
        lw_rain = getattr(lw, "rain", 1.0)

        # 2. 🌟 动态匹配损失模式 (MAE / MSE Alignment)
        loss_cfg = self.train_cfg.get("loss", {})
        mode = str(loss_cfg.get("mode", "mse")).lower()

        # 雷达与卫星常规使用 MSE 评估
        loss_radar = F.mse_loss(pred_target["radar"], target["radar"])
        loss_satellite = F.mse_loss(pred_target["satellite"], target["satellite"])

        # 降水（Rain）根据训练配置匹配 L1 (MAE) 或 L2 (MSE)
        if mode == "mae":
            loss_rain = F.l1_loss(pred_target["rain"], target["rain"])
        else:
            loss_rain = F.mse_loss(pred_target["rain"], target["rain"])

        # 汇总验证阶段的推理损失 (Rollout Loss)
        infer_loss = (
                lw_radar * loss_radar
                + lw_satellite * loss_satellite
                + lw_rain * loss_rain
        )

        # 3. 记录日志与后续 Block 的单帧验证
        extra_logs: dict[str, torch.Tensor] = {
            "val/loss_radar": loss_radar.detach(),
            "val/loss_satellite": loss_satellite.detach(),
            "val/loss_rain": loss_rain.detach(),
        }
