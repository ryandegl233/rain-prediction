...
def time_shift_func(t: torch.Tensor, flow_shift: float = 1.0, sigma: float = 1.0):
...
class TransEncoder(nn.Module):
    def __init__(
        ...
        for i in range(num_res_blocks):
            self.res_blocks.append(TransBlock(model_channels, use_swiglu))
        ...
        for i in range(num_ada_ln_blocks):
            self.ada_ln_blocks.append(nn.Linear(model_channels, model_channels * 6, bias=True))
        self.ada_ln_switch_freq = max(1, num_res_blocks // num_ada_ln_blocks)
        assert (num_res_blocks % self.ada_ln_switch_freq) == 0, "num_res_blocks must be divisible by num_ada_ln_blocks"
