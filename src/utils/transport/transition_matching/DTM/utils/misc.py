...
def dict_to_str(d):
    ...
    def func(k, v):
        return f"{k}: {v.item() if isinstance(v, torch.Tensor) else v}"
...
def model_load(path: str, model: nn.Module, strict: bool = True, device: str = None) -> nn.Module:
...
def concat_dif_timesteps_tensor(tensor_list, n: int, out_size):
    cat_tensor = list_tensor2_list_list(tensor_list, n, mode="size")
    ts = []
    for i in range(n):
        t = cat_tensor[i]
        t = torch.cat(t, dim=1)
        t = nn.functional.interpolate(t, out_size, mode="bilinear", align_corners=True)
        ts.append(t)
...
def img_batch2one_img(batched_img: torch.Tensor) -> torch.Tensor:
    ...
    if batched_img.shape[1] > 3:
        ...
        s = slice(None)
    img = tv.utils.make_grid(batched_img.detach().cpu(), nrow=int(b**0.5), padding=0, normalize=True).permute(1, 2, 0)[
        ..., s
    ]
    return img
...
def compute_iters(size, bs, drop_last=False):
    ...
    last = int((fp_iters - int_iter) > 0.0) if not drop_last else 0
