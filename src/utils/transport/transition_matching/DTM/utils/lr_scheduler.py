...
class WarmupCosineSchedule(object):
    ...
    def get_lr(self):
        ...
        return [
            base_lr
            * 0.5
            * (1.0 + math.cos(math.pi * (self.last_epoch - self.warmup_steps) / (self.t_total - self.warmup_steps)))
...
class WarmupLinearScheduler(object):
    ...
    def __init__(self, optimizer, warmup_steps, t_total, last_epoch=-1, only_warmup=False):
        self.optimizer = optimizer
        self.warmup_steps = warmup_steps
        self.t_total = t_total
        self.last_epoch = last_epoch
    ...
    def get_lr(self):
        ...
        if not self.only_warmup:
            return [
                base_lr * (self.t_total - self.last_epoch) / (self.t_total - self.warmup_steps)
                for base_lr in self.base_lrs
            ]
        else:
...
if __name__ == "__main__":
    ...
    scheduler = MultiStepConstantScheduler(optim, [10, 50, 100], [0.001, 0.0001, 0.00001])
    lrs = []
    for i in range(200):
        scheduler.step()
