...
class SpectralBallFusedParamGroups:
    spectralball: dict[str, Any]
    oned: dict[str, Any]
...
def split_spectralball_oned_params(
    named_params: Iterable[tuple[str, torch.nn.Parameter]] | dict[str, torch.nn.Parameter],
    ignored_keys_for_spectralball: tuple[str, ...] | list[str] = (),
    oned_param_algo: str = "adamw",
) -> SpectralBallFusedParamGroups:
    if oned_param_algo not in ("adamw", "lion"):
        raise ValueError(f"oned_param_algo must be one of: adamw, lion. Got: {oned_param_algo}")

    spectralball_params: list[torch.nn.Parameter] = []
    oned_params: list[torch.nn.Parameter] = []

    ignore_patterns = _compile_patterns(ignored_keys_for_spectralball)
    for name, p in _iter_named_parameters(named_params):
        if not p.requires_grad:
            continue

        if p.ndim == 2 and not _should_ignore_param(name, ignore_patterns):
            spectralball_params.append(p)
        else:
            oned_params.append(p)

    spectralball_group: dict[str, Any] = {"params": spectralball_params, "algorithm": "spectralball"}
    oned_group: dict[str, Any] = {"params": oned_params, "algorithm": oned_param_algo}
    return SpectralBallFusedParamGroups(spectralball=spectralball_group, oned=oned_group)


class SpectralBallFused(Optimizer):
    def __init__(
        self,
        params: ParamsT,
        *,
    ...
    @classmethod
    @function_config_to_basic_types
    def create_spectralball_optimizer(
        cls,
        named_parameters: Iterable[tuple[str, torch.nn.Parameter]] | dict[str, torch.nn.Parameter],
        ignored_keys_for_spectralball: tuple[str, ...] | list[str] = (),
        *,
        oned_param_algo: str = "adamw",
        spectralball_params_defaults: dict[str, Any] | None = None,
        oned_params_defaults: dict[str, Any] | None = None,
        **defaults: Any,
    ) -> SpectralBallFused:
        groups = split_spectralball_oned_params(
            named_parameters,
            ignored_keys_for_spectralball=ignored_keys_for_spectralball,
            oned_param_algo=oned_param_algo,
        )

        param_groups: list[dict[str, Any]] = []
        if groups.spectralball["params"]:
            g = dict(groups.spectralball)
            if spectralball_params_defaults:
                g.update(spectralball_params_defaults)
    ...
    def step(self, closure: Any | None = None) -> float | None:
        ...
        for group in self.param_groups:
            group["step"] += 1
            algo = group.get("algorithm", "spectralball")
            if algo == "spectralball":
                self._step_spectralball_group(group)
            elif algo == "adamw":
    ...
    def _step_spectralball_group(self, group: dict[str, Any]) -> None:
        lr = float(group["lr"])
        ...
        solver = cast(str, group["solver"])
        solver_tolerance_f = float(group["solver_tolerance_f"])
        solver_max_iterations = int(group["solver_max_iterations"])
        radius_mode = cast(str, group["radius_mode"])
        radius_scaler = float(group.get("radius_scaler", 1.0))
        ...
        for p in group["params"]:
            if p.grad is None:
                continue
            if p.ndim != 2:
                raise ValueError("SpectralBallFused spectralball group only supports 2D parameters.")
            ...
            update, _, _ = compute_spectral_ball_update(
                W=p.data,
                M=grad_eff,
