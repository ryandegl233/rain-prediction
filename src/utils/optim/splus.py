    ...
    def __setstate__(self, state):
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                step_val = float(p_state.get("step", 0))
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                if len(p.shape) == 2:
                    ...
                    state["sides"][1] = (
                        torch.lerp(state["sides"][1], grad.T @ grad, 1 - group["b2"])
...
if __name__ == "__main__":
    ...
    optimizer = SPlus(network.parameters(), lr=0.1)
    for _ in range(10):
    ...
    print("Optimizer state dict:", state_dict)
