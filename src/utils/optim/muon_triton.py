...
def muon_update(grad, momentum, beta=0.95, ns_steps=5, nesterov=True, rms_scale=False):
    ...
    momentum.lerp_(grad, 1 - beta)
    _orig_g_shape = grad.shape
    update = grad.lerp_(momentum, beta) if nesterov else momentum
    if update.ndim > 2:
        update = update.view(grad.size(0), -1)
    update = fast_newtonschulz(update, steps=ns_steps)
    update = apply_scaling(update.view(_orig_g_shape), rms_scale)
    return update
...
class Muon(torch.optim.Optimizer):
    ...
    def clear_muon_adamw_params(
        ...
        return {"params": muon_params, "use_muon": True}, {
            "params": adamw_params,
            "use_muon": False,
        }
...
if __name__ == "__main__":
    network = torch.nn.Sequential(
        ...
        torch.nn.Conv2d(16, 3, 1, 1),
    ).cuda()
    x = torch.randn(1, 3, 256, 256).cuda()

    pg = Muon.clear_muon_adamw_params(network.named_parameters())
    optim = Muon(pg, defaults=dict(lr=1e-3))
